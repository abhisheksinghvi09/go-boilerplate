...
import (
	...
	"github.com/abhisheksinghvi09/go-boilerplate/internal/database"
	"github.com/abhisheksinghvi09/go-boilerplate/internal/lib/job"
	loggerPkg "github.com/abhisheksinghvi09/go-boilerplate/internal/logger"
...
func New(cfg *config.Config, logger *zerolog.Logger, loggerService *loggerPkg.LoggerService) (*Server, error) {
	...
	if err := redisClient.Ping(ctx).Err(); err != nil {
		logger.Error().Err(err).Msg("Failed to connect Redis, continuing without Redis")
	...
	// Runtime metrics are automatically collected by New Redlic Go agent
