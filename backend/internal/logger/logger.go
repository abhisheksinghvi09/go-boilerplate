...
import (
	...
	"github.com/abhisheksinghvi09/go-boilerplate/internal/config"
...
func NewLogger(level string, isProd bool) zerolog.Logger {
	return NewLoggerWithSerice(&config.ObservabilityConfig{
...
func NewloggerWithConfig(cfg *config.ObservabilityConfig) zerolog.Logger {
	return NewLoggerWithSerice(cfg, nil)
}

// NewLoggerWithService creates a logger with full config and logger service
func NewLoggerWithSerice(cfg *config.ObservabilityConfig, loggerService *LoggerService) zerolog.Logger {
	...
	if cfg.IsProduction() && cfg.Logging.Format == "json" {
		...
		// Development node - use console writer
		consoleWriter := zerolog.ConsoleWriter{Out: os.Stdout, TimeFormat: "2006-01-02 15:04:05"}
		writer = consoleWriter
	}

	// Note: New Relic log forwarding is now handled automatically by zerlogwriter integration
...
// WithTracerContext adds New Relic transaction context to logger
func WithTracerContext(logger zerolog.Logger, txn *newrelic.Transaction) zerolog.Logger {
	if txn == nil {
		return logger
	}
	...
	writer := zerolog.ConsoleWriter{
		...
		FormatFieldValue: func(i any) string {
			...
			case string:
				...
				if len(v) > 200 {
					// Truncate very logn SQL statements
					return v[:200] + "..."
				}
				return v
			case []byte:
